    ...
    fun calculatePaths(input: List<String>, part2:Boolean = false): Int {
        ...
        while(pathPermute.isNotEmpty()) {
            ...
            if (curPath.last() == "end") {
                ...
                if (!part2 && groupByCount(curPath.filter{it == it.lowercase()}).any { it.value > 1 }) continue
                ...
                nextPaths.forEach { it: String ->
                    if (
                        !curPath.contains(it) ||
                        it == it.uppercase() ||
                        part2 && it != "start" && it != "end" && groupByCount(curPath.filter{it == it.lowercase()}).all{it.value < 2}) {
...
class Cave(
    ...
    val isSmallCave = id == id.lowercase()
}
